...
class ucp_reactions
{
	...
	public function handle($id, $mode)
	{
		...
		$sql = 'SELECT user_reactions_notify, user_reactions_cron_email
		...
		$current_notify = (bool) ($row['user_reactions_notify'] ?? 1);
		$current_email = (bool) ($row['user_reactions_cron_email'] ?? 1);
		...
		$this->template->assign_vars(array(
			...
			'UCP_REACTIONS_CRON_EMAIL'	=> $current_email,
		));
